...
async function 保存键值配置(候选配置 = 键值配置) {
  if (!键值存储) throw new Error('KV storage is not configured');
  const 存储 = 键值存储;
  const 快照 = { ...候选配置 };
  await 存储.put('c', JSON.stringify(快照));
  键值配置 = 快照;
  const 新版本 = String(Date.now());
  try {
    await 存储.put('c_ver', 新版本);
    键值配置版本 = 新版本;
  } catch {
    // Data was saved; a version-marker failure must not claim the data was lost.
    键值配置版本 = '';
  }
  键值配置上次加载 = Date.now();
}
...
export default {
  async fetch(请求735, 本地值734, 本地值733) {
    try {
      ...
      // 加载自定义DNS和ECH域名配置
      自定义域名系统 = 获取配置文本值('customDNS', 配置默认值.customDNS, 读取环境配置值(本地值734, 'customDNS', 'CUSTOMDNS', 'CUSTOM_DNS')).trim() || 配置默认值.customDNS;
      自定义加密客户端问候域名 = 获取配置文本值('customECHDomain', 配置默认值.customECHDomain, 读取环境配置值(本地值734, 'customECHDomain', 'CUSTOMECHDOMAIN', 'CUSTOM_ECH_DOMAIN')).trim() || 配置默认值.customECHDomain;
...
<body>
    ...
    document.addEventListener('DOMContentLoaded', function() {
      ...
      if (输入) {
        ...
        const 基础路径 = '/' + (自定义路径 && 自定义路径.trim() ? 自定义路径.trim() : 认证令牌).replace(/^\/+|\/+$/g, '');
        const 请求路径 = 网址698.pathname.replace(/\/+$/, '') || '/';
        if (请求路径 === 基础路径) return await 处理订阅值(请求735, 认证令牌);
        if (请求路径 === 基础路径 + '/sub') return await 处理订阅请求(请求735, 认证令牌, 网址698);
      }
...
async function 处理网页套接字请求(请求) {
  const 参数 = new URL(请求.url).searchParams;
  let 代理 = 是否代理已启用 ? { ...已解析代理5配置 } : null;
  if (参数.has('s')) {
    try { 代理 = 解析代理配置(参数.get('s')); }
    catch { return new Response('Invalid proxy configuration', { status: 400 }); }
  }
  if (仅走代理 && !代理) return new Response('Proxy-only mode requires a valid proxy', { status: 503 });
  const 回退文本 = 参数.get('p') || 回退地址;
  const 回退 = 回退文本 ? 解析地址值端口(回退文本) : null;
  const 选项 = {
    token: 认证令牌,
    proxy: 代理,
    proxyOnly: 仅走代理,
    directFirst: 启用代理降级,
    fallback: 回退 ? { hostname: 回退.address, port: 回退.port } : null,
    earlyData: 请求.headers.get('sec-websocket-protocol') || '',
...
async function 处理配置接口(请求54, 环境值 = {}) {
  if (请求54.method === 'GET') {
    ...
    try {
      ...
      if (!新配置 || typeof 新配置 !== 'object' || Array.isArray(新配置)) throw new Error('Invalid configuration object');
      const 候选配置 = { ...键值配置 };
